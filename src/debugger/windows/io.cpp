...
void IO::render() {
    ...
    ImGui::NextColumn();
    const char *lcdc_labels[8] = {"LCD Enable",  "Window Tile Map", "Window Display", "BG Tile Data",
                                  "BG Tile Map", "OBJ Size",        "OBJ Display",    "BG Display"};
    const uint8_t lcdc_bits[8] = {LCDC_LCD_ENABLE,  LCDC_WINDOW_TILE_MAP, LCDC_WINDOW_DISPLAY, LCDC_BG_WINDOW_TILE_DATA,
                                  LCDC_BG_TILE_MAP, LCDC_OBJ_SIZE,        LCDC_OBJ_DISPLAY,    LCDC_BG_DISPLAY};
    draw_registers(lcdc_labels, LCDC, lcdc_bits, 8);

    ImGui::NextColumn();
    const char *stat_labels[7] = {"Coincidence Interrupt", "OAM Interrupt", "V-Blank Interrupt", "H-Blank Interrupt",
                                  "Coincidence Flag",      "Mode (bit 1)",  "Mode (bit 0)"};
    const uint8_t stat_bits[7] = {STAT_COINCID_INT,  STAT_OAM_INT, STAT_VBLANK_INT, STAT_HBLANK_INT,
